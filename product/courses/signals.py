"""
На данном этапе появляются вопросы по бизнес-логике:
    1. Если курс уже начался (группы не пересобрать) и все группы полные,
    то нового студента придется записывать в любую группу, игнорируя
    максимальное допустимое количество.
    Ведь иметь группу с одним человеком не логично.
    2. Если минимальное и максимальное допустимое количество студентов
    в группе не сильно отличаются (например min=15 и max=20), то невозможно
    пересобрать 2 группы учитывая минимальное допустимое количество, условие
    разницы участников групп в 1 человека и распределение по-умолчанию
